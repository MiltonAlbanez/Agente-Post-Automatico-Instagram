...
from .config import load_config, init_runtime_monitoring, perform_preflight_checks
...
def main():
    # Inicializar monitoramento e logs
    try:
        init_runtime_monitoring()
    except Exception:
        pass
    ...
    if args.cmd == "collect":
        ...
        cfg = load_config()
        # Pré-validações para o horário crítico de 19h BRT
        print("🔎 Pré‑validações para postagem automática:")
        pre_ok = False
        try:
            pre_ok = perform_preflight_checks(cfg)
        except Exception as e:
            print(f"⚠️ Falha nas pré‑validações: {e}")
        print(f"✅ Pré‑validações: {'OK' if pre_ok else 'COM AVISOS'}")
        ...
        if not rows:
            ...
            # Usar imagem aleatória do Unsplash por tema para evitar repetição
            source_image_url = get_random_unsplash_url(fallback_theme)
        ...
        for acc in accounts:
            ...
            if (rapidapi_failed or force_fallback_stories) and is_stories_mode:
                ...
                # Imagem aleatória do Unsplash por tema para fallback
                fallback_image = get_random_unsplash_url(selected_theme)
        ...
        # Imagem aleatória do Unsplash por tema (single-run)
        theme = args.theme.lower() if args.theme else "motivacional"
        source_image_url = get_random_unsplash_url(theme)
...
def get_random_unsplash_url(theme: str) -> str:
    """Retorna uma URL aleatória do Unsplash adequada ao tema.
    Usa o endpoint de random com palavras‑chave do tema e um 'sig' aleatório para variar.
    """
    theme = (theme or "motivacional").lower()
    keyword_map = {
        "motivacional": "motivation,success,goal",
        "produtividade": "productivity,focus,workspace",
        "lideranca": "leadership,team,business",
        "mindset": "mindset,thinking,brain",
        "negocios": "business,entrepreneur,office",
    }
    query = keyword_map.get(theme, keyword_map["motivacional"])
    sig = random.randint(1, 10**9)
    # Gera 1080x1080 para manter compatibilidade com Instagram
    return f"https://source.unsplash.com/1080x1080/?{query}&sig={sig}"
