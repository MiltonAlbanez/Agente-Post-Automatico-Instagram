...
class InstagramClientRobust:
    ...
    # =====================
    # Métodos específicos para Stories (robustos)
    # =====================
    def prepare_stories_media(self, image_url: str) -> str:
        """Prepara mídia para Stories com retry/timeout robustos"""
        url = f"{self.BASE}/{self.business_account_id}/media"
        params = {
            "image_url": image_url,
            "media_type": "STORIES",
    ...
    def poll_stories_media_status(self, media_id: str, interval_sec: int = 10, max_checks: int = 60) -> str:
        """Verifica o status da mídia do Stories com robustez"""
        url = f"{self.BASE}/{media_id}"
        params = {"fields": "status_code,status", "access_token": self.access_token}
        status = ""
        for check in range(max_checks):
    ...
    def publish_stories_media(self, creation_id: str) -> str:
        """Publica mídia no Stories com robustez"""
        url = f"{self.BASE}/{self.business_account_id}/media_publish"
        params = {"creation_id": creation_id, "access_token": self.access_token}
        resp = self._make_request_with_retry('POST', url, params, timeout=120)
        if not resp.ok:
    ...
    def poll_stories_published_status(self, media_id: str, interval_sec: int = 10, max_checks: int = 60) -> str:
        """Verifica status da publicação do Stories com robustez"""
        url = f"{self.BASE}/{media_id}"
    ...
    def publish_to_stories_complete(self, image_url: str) -> dict:
        """Fluxo completo para publicar no Stories com robustez"""
        start_time = datetime.now()
        try:
