...
class BackupManager:
    ...
    def __init__(self, config_path: str = "config/backup_config.json", enable_scheduler: bool = True):
    ...
    def _setup_scheduler(self) -> None:
        ...
        # Backup diário
        daily_time = self.config.get("schedule", {}).get("daily_backup", "02:00")
        schedule.every().day.at(daily_time).do(self.create_daily_backup)

        # Backup semanal completo por dia da semana (ex.: sunday)
        weekly_day = str(self.config.get("schedule", {}).get("weekly_full_backup", "sunday")).lower()
        weekly_job = getattr(schedule.every(), weekly_day, None)
        if weekly_job is not None and hasattr(weekly_job, "at"):
            weekly_job.at(daily_time).do(self.create_full_backup)
        else:
            # Fallback seguro se configuração inválida
            schedule.every().sunday.at(daily_time).do(self.create_full_backup)

        # Limpeza "mensal" emulada: verifica diariamente se é o dia configurado
        monthly_day = int(self.config.get("schedule", {}).get("monthly_cleanup", 1))
        schedule.every().day.at(daily_time).do(lambda: self._monthly_cleanup_if_due(monthly_day))

    def _monthly_cleanup_if_due(self, day: int) -> None:
        try:
            if datetime.now().day == day:
                self.cleanup_old_backups()
        except Exception as e:
            print(f"❌ Erro na verificação de limpeza mensal: {e}")
    ...
    def _backup_configs(self, backup_path: Path) -> None:
        ...
        try:
            ...
            ignore_patterns = self.config.get("targets", {}).get("configs", {}).get("ignore_patterns", [])
            default_ignores = ["*.lock", "*.tmp", "*.swp", "__pycache__", ".DS_Store", "Thumbs.db"]
            all_ignores = set(ignore_patterns + default_ignores)
            ...
            for config_path in self.config["targets"]["configs"]["paths"]:
                ...
                if source_path.exists():
                    if source_path.is_file():
                        ...
                        # Se já existir como arquivo, remover para permitir diretório
                        if dest_dir.exists() and dest_dir.is_file():
                            try:
                                dest_dir.unlink()
                            except Exception as e:
                                print(f"⚠️ Não foi possível remover arquivo existente '{dest_dir}': {e}")
                                continue
                        # Copiar diretório com tratamento de permissão e ignores
                        self._copy_dir_safe(source_path, dest_dir, all_ignores)
    ...
    def _copy_dir_safe(self, src: Path, dest: Path, ignore_patterns) -> None:
        """Copia diretório ignorando padrões e continuando em erros de permissão"""
        try:
            ...
            for root, dirs, files in os.walk(src):
                ...
                pruned_dirs = []
                for d in list(dirs):
                    rel_path = str((rel_root / d).as_posix())
                    if any(fnmatch.fnmatch(rel_path, pat) or d.startswith('.') for pat in ignore_patterns):
                        # Ignorar diretório
                        continue
                    pruned_dirs.append(d)
                # Atualizar diretórios para evitar descer nos ignorados
                dirs[:] = pruned_dirs

                # Copiar arquivos
                for f in files:
                    rel_path = str((rel_root / f).as_posix())
                    if any(fnmatch.fnmatch(rel_path, pat) or f.startswith('.') for pat in ignore_patterns):
                        continue
                    src_file = Path(root) / f
                    dest_file_dir = dest / rel_root
                    dest_file_dir.mkdir(parents=True, exist_ok=True)
                    dest_file = dest_file_dir / f
                    try:
                        shutil.copy2(src_file, dest_file)
                    except PermissionError as e:
                        print(f"⚠️ Permissão negada ao copiar '{src_file}': {e}")
                    except Exception as e:
                        print(f"⚠️ Falha ao copiar '{src_file}': {e}")
        except Exception as e:
            print(f"❌ Erro na cópia segura de diretório '{src}': {e}")
